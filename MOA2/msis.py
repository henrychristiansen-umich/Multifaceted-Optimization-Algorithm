...
def Run_MSIS(filename, swarm, start_date, end_date):
    ...
    for i, time in enumerate(dates):
        ...
        if day not in data or time not in swarm:
            continue
...
def Run_Comparison(uncorrected, corrected, swarm, start_date, end_date):
    ...
    for start in np.arange(0, len(times), period):
        ...
        swarm_avg.append(np.nanmean(swarm_density[start:end]))
        moa_avg.append(np.nanmean(moa_density[start:end]))
        uncorrected_avg.append(np.nanmean(uncorrected_density[start:end]))
    ...
    plt.title(f"SWARM {SWARM}: Orbit-averaged Nuetral Densities for {start_date.astype('datetime64[M]')}")
    plt.xlim(start_date, end_date)
    plt.tight_layout()
    plt.legend()
    plt.savefig(f'../STORMS/{STORM}/Results/swarm{SWARM}.png')

    print("-------")

    print("RMS ERROR - MSIS: " + str(np.sqrt(np.mean((np.array(swarm_avg) - np.array(uncorrected_avg)) ** 2))))
    print("RMS ERROR - MOA : " + str(np.sqrt(np.mean((np.array(swarm_avg) - np.array(moa_avg)) ** 2))))
...
origninal_cssi = "/home/hennyc/GMAT/data/atmosphere/earth/SpaceWeather-All-v1.2.txt"
fopt_output = "../STORMS/" + STORM + "/WEATHER.txt"
champ = "../STORMS/" + STORM + f"/DENSITY_DATA/SWARM{SWARM}.txt"
