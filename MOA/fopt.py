...
def find_adjustments(args):
    ...
    try:
        ...
        for _ in range(10):
            ...
            if last_error is not None:
                if np.all(abs(last_error-error) < 1):
                    break
...
def update_ap(ap, time, error):
    ...
    # 80 points over 10 days → 3-hour resolution = 0.125 days
    ap_time = np.linspace(0, 10, len(ap))

    # Interpolate the error over the full 10-day period
    # Fill outside the range with nearest edge values (or 0)
    interp_error = np.interp(ap_time, time, error, left=error[0], right=error[-1])

    # Apply scaling parameters (alpha, beta, gamma)
    alpha = 0.4
    ap_adjustment = -alpha * interp_error
