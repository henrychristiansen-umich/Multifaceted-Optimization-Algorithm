# Mass (Ballistic Coefficeint) Solver for inputted spacecraft
#
# Coded by Henry Christiansen. University of Michigan.
#
# This file solves for the mass of spacecraft given TLE data
...
def read_tle_file(file, start_date, end_date, sigma):
    ...
    for id in tqdm(raw_data.keys(), desc="Filtering"):
        ...
        for i in tle_list:
            ...
            too_close = any(abs((dt - existing).total_seconds()) < 3 * 3600 for existing in epochs)
            if too_close:
                continue
            epochs.append(dt)
        ...
        if len(tle_sma) < int(((end_date - start_date) / np.timedelta64(1, 'D'))/4):
            continue
    ...
    if False:
        ...
        # Plot all dsma traces (unfiltered) with low opacity
        for times_arr, dsma_arr in zip(sma_times, dsma_values):
            if len(times_arr) > 0:
                plt.plot(times_arr, dsma_arr, marker='o', color='black', alpha=0.05, linewidth=1)

        # Plot filtered traces with full opacity
        for (id, valid_tle), times_arr, dsma_arr in zip(ids, sma_times, dsma_values):
            if id in filtered_data:
                if len(times_arr) > 0:
                    plt.plot(times_arr, dsma_arr, marker='o',color='black', alpha=1.0, linewidth=5.0)

        # Plot median step line in red
        bin_medians = [b[0] for b in bin_stats]
        bin_dates = bin_edges[:-1]  # days since start_date
...
def find_mass(args):
    ...
    #             log.write(f"Starting mass solve for {sat_id}\n")
...
if __name__ == '__main__':
    ...
    filter_sigma = 1
    max_iterations = 25
    mass_tolerance = 0.01 # kg

    with open(f"{base_path}/DATES.txt", "r") as file:
    ...
    gmat_script_path = 'newmopt.script'

    data = read_tle_file(tle_path, np.datetime64(start_str_1), np.datetime64(end_str_2), filter_sigma)
    ids = list(data.keys())
